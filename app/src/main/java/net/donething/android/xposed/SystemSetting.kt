...
class SystemSetting {
    companion object {
         ...
         * 应用安装完成后，发送通知
         */
        fun makeNotification(activity: Activity, title: String, text: String, packageName: String, isInstallSuccess: Boolean) {
            try {
         ...
         * 判断是否已是最小音量（非无声）
         */
        fun isMinVolume(old: Int, current: Int): Boolean {
            // 音量值不相等，且最小音量为0，或
        ...
        fun dealEnableHierarchy(lpparam: XC_LoadPackage.LoadPackageParam) {
            ...
            findAndHookMethod("com.android.server.wm.WindowManagerService", lpparam.classLoader, "isSystemSecure", object : XC_MethodHook() {
                override fun beforeHookedMethod(param: MethodHookParam) {
